...
class DynamicSimpleSGLangSelector:
    ...
    def __init__(
        ...
        # Initialize tracking variables for incremental verification (needed for warm up)
        self.last_reference_positions = []
    ...
    def warm_up_reference_model(self):
        ...
        # Initialize last_reference_positions for warm up
        if not self.last_reference_positions:
            self.last_reference_positions.append(0)
    ...
    def reference_model_worker(rank, world_size: int, base_gpu_id: int, server_args: ServerArgs, input_queues: List[mp.Queue], output_queue: mp.Queue, ack_queue: mp.Queue):
        ...
        while True:
            ...
            if isinstance(reqs, int):
                ...
                DynamicSimpleSGLangSelector.simple_prepare_for_verify(new_batch)
                batch = new_batch.get_model_worker_batch()
                _, next_token_ids = scheduler.tp_worker.forward_batch_generation(batch)
                next_token_ids_list = next_token_ids.tolist() 
                next_token_ids_answer = next_token_ids_list[-1:]

                if rank == 0:
                    output_queue.put(next_token_ids_answer)
    ...
    @staticmethod
    def simple_prepare_for_verify(batch: ScheduleBatch):
        """
        Prepare batch for TARGET_VERIFY mode execution.
        This method sets up incremental KV cache computation similar to EAGLE's verify mechanism.
        Only new tokens (since last reference model usage) will have their KV cache computed.
        """
        global end_of_cache_loc
        ...
        for r in reqs:
            ...
            fill_len = len(r.fill_ids)
            new_tokens = r.fill_ids[-r.extend_input_len:]
            input_ids.append(new_tokens)
        ...
        for r in reqs:
            fill_len = len(r.fill_ids) if r.fill_ids is not None else 0
            prefix_len = len(r.prefix_indices) if r.prefix_indices is not None else 0
            seq_lens.append(fill_len)
        ...
        extend_lens_tensor = torch.tensor(extend_lens, dtype=torch.int64).to(
            batch.device, non_blocking=True
        )
        ...
        batch.extend_lens_tensor = extend_lens_tensor  
        ...
        # Build spec info 
        batch.spec_algorithm = SpeculativeAlgorithm.EAGLE3
        draft_token = input_ids_tensor
        draft_num_token = len(draft_token)

        positions_list = []
        for r in reqs:
            start_pos = len(r.prefix_indices)
            end_pos = start_pos + r.extend_input_len
            positions_list.append(torch.arange(start_pos, end_pos, dtype=torch.long, device=batch.device))
        positions = torch.cat(positions_list) if len(positions_list) > 1 else positions_list[0]
        
        retrive_index = torch.arange(0, draft_num_token, dtype=torch.long, device=batch.device).unsqueeze(0) 

        retrive_next_token = torch.cat([
            torch.arange(1, draft_num_token, dtype=torch.long, device=batch.device),
            torch.tensor([-1], dtype=torch.long, device=batch.device)
        ]).unsqueeze(0) 
        
        retrive_next_sibling = torch.full((1,draft_num_token), -1, dtype=torch.long, device=batch.device)
        ...
        batch.spec_info = EagleVerifyInput(
            ...
            draft_token_num=draft_num_token,
            spec_steps=draft_num_token-1,
            capture_hidden_mode=CaptureHiddenMode.FULL
        )
        #batch.output_ids = [draft_token[0].item()]
        
        # Important: For speculative decoding, the setting of seq_lens is different from that in autoregressive mode
        new_seq_lens = []
        for seq_len in seq_lens:
            new_seq_lens.append(seq_len - draft_num_token)
        new_seq_lens_tensor = torch.tensor(new_seq_lens, dtype=torch.int64).to(
            batch.device, non_blocking=True
        )
        batch.seq_lens = new_seq_lens_tensor
    ...
    def extend_step(self, input_ids: List[List[int]], input_indices: List[int], sampling_params: SamplingParams) -> List[int]:
        """
        NEW IMPLEMENTATION: Use reference model with TARGET_VERIFY
        
        This method uses TARGET_VERIFY mode to update KV cache incrementally (from last reference generation to current position, excluding last token), then generate next token via scheduler
        """
        reqs = []
        
        for i, (input_id, seq_idx) in enumerate(zip(input_ids, input_indices)):
            # Ensure last_reference_positions has enough elements
            while len(self.last_reference_positions) <= seq_idx:
                self.last_reference_positions.append(0)
            
            last_ref_pos = self.last_reference_positions[seq_idx]
            tokens_to_verify = input_id[last_ref_pos:]
            input_text = self.tokenizer.decode(input_id)
            req = Req(
                rid=seq_idx,
            ...
            req.prefix_indices = list(range(last_ref_pos)) if last_ref_pos > 0 else []
            req.fill_ids = input_id
            req.extend_input_len = len(tokens_to_verify)
            
            reqs.append(req)

        # Send requests to reference model workers
        for q in self.reference_model_input_queues:
            q.put_nowait(reqs)
        
        next_token_ids = self.reference_model_output_queue.get()

        # Update last reference positions for each sequence
        for i, seq_idx in enumerate(input_indices):
            self.last_reference_positions[seq_idx] = len(input_ids[i])
    ...
    def generate(
        ...
        # Track last reference model position for each sequence for incremental verification
        self.last_reference_positions = [0 for _ in range(batch_size)]
    ...
    def reset_cache_simple(self):
        """Reset the cache for the quick model and tracking variables"""
        for q in self.reference_model_input_queues:
        ...
        # Reset tracking variables for incremental verification
        if hasattr(self, 'last_reference_positions'):
            self.last_reference_positions = []
